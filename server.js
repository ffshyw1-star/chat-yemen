...
// إنشاء المجلدات التلقائية لتخزين البيانات والملفات المرفوعة
...
// إعدادات multer لحفظ المرفقات والصوتيات المرفوعة بشكل صحيح
const imgStorage = multer.diskStorage({ 
  destination: './uploads/', 
  filename: (req, f, cb) => cb(null, 'img_' + Date.now() + path.extname(f.originalname)) 
});
const audioStorage = multer.diskStorage({ 
  destination: './uploads/', 
  filename: (req, f, cb) => cb(null, 'audio_' + Date.now() + '.webm') 
});
...
app.post('/upload-audio', uploadAudio, (req, res) => {
...
io.on('connection', (socket) => {
  ...
  if (bannedIPs[ip] && (bannedIPs[ip].expire === null || bannedIPs[ip].expire > Date.now())) {
    socket.emit('banned', `أنت محظور من السيرفر بسبب: ${bannedIPs[ip].reason}`);
  ...
  socket.on('join', (u) => {
    ...
    users[socket.id] = { ...u, id: socket.id, room: 'عام', isAdmin, ip, color: '#000000', font: 'Tajawal' };
    socket.join('عام'); 
  ...
  socket.on('message', (d) => {
    const user = users[socket.id]; 
    if (!user || globalMuted.includes(socket.id)) return;
  ...
  socket.on('mute user', (id) => { 
    if (users[socket.id]?.isAdmin && !globalMuted.includes(id)) { 
      globalMuted.push(id); 
      saveMuted(); 
      io.to(id).emit('you muted', true); 
      if (users[id]) io.to(users[socket.id].room).emit('system', `${users[id].name} تم كتمه من قبل الإدارة`); 
    } 
  });
  
  socket.on('unmute user', (id) => { 
    if (users[socket.id]?.isAdmin) { 
      globalMuted = globalMuted.filter(x => x !== id); 
      saveMuted(); 
      io.to(id).emit('you muted', false); 
      if (users[id]) io.to(users[socket.id].room).emit('system', `تم فك الكتم عن العضو ${users[id].name}`); 
    } 
  });
  
  socket.on('kick user', (id) => { 
    if (users[socket.id]?.isAdmin) { 
      io.to(id).emit('kicked', 'تم طردك من الغرفة من قبل الإدارة'); 
  ...
  socket.on('ban user', (d) => {
    ...
    const t = users[d.targetId]; 
    if (!t) return;
    
    bannedIPs[t.ip] = { reason: d.reason || 'مخالفة القوانين', expire: d.hours ? Date.now() + d.hours * 3600000 : null, by: users[socket.id].name };
    saveBans();
    
    io.to(d.targetId).emit('banned', `تم حظرك نهائياً: ${bannedIPs[t.ip].reason}`);
    io.sockets.sockets.get(d.targetId)?.disconnect(true);
    io.to(users[socket.id].room).emit('system', `تم حظر العضو ${t.name} حظراً ${d.hours ? d.hours + ' ساعة' : 'دائماً'}`);
  });
  ...
  socket.on('unban ip', (ip) => { 
    if (users[socket.id]?.isAdmin) { 
      delete bannedIPs[ip]; 
      saveBans(); 
      io.to(users[socket.id].room).emit('system', `تم فك الحظر عن الأيبي ${ip}`); 
    } 
  });
  
  socket.on('get bans', () => { 
    if (users[socket.id]?.isAdmin) socket.emit('bans list', bannedIPs); 
  });
  
  socket.on('save settings', (s) => { 
    if (users[socket.id]) { 
      users[socket.id].font = s.font; 
      users[socket.id].color = s.color; 
    } 
  });
  ...
  socket.on('disconnect', () => { 
    ...
    if (u) { 
      rooms[u.room] = rooms[u.room].filter(x => x !== socket.id); 
...
server.listen(3000, () => console.log(`السيرفر يعمل بكفاءة على الرابط: http://localhost:3000`));
